...
export const play = async (playbackObj, uri, lastPosition) => {
  try {
    ...
    //if there's last position then play audio from last position
...
export const moveAudio = async (context, value) => {
  ...
  try {
    ...
    updateState(context, {
      soundObj: status,
      playbackDuration: status.positionMillis,
    });

    await resume(playbackObj);
  } catch (error) {
    console.log("error insisde onSlidingComplete callback", error.message);
  }
};
